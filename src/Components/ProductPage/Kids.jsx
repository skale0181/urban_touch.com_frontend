...
import { getKids, getKidsData, getKidsFilterData } from "../../Redux/Kids/action";
...
export const Kids = () => {
  ...
  const products = useSelector((store) => store.kids.data);
  const {loading,error} = useSelector((store) => store.kids)
  ...
  const handlefilter = (val) => {
    if (val === 1) {
      ...
      dispatch(getKidsFilterData(1));
    } else if (val === 2) {
      const x = products.filter((item) => {
        return item.price > 100 && item.price <= 300;
      });
      dispatch(getKidsFilterData(2));
    } else if (val === 3) {
      const x = products.filter((item) => {
        return item.price >= 300;
      });
      dispatch(getKidsFilterData(3));
    }else if (val === 4) {
      const x = products.filter((item) => {
        return item.price >= 300;
      });
      dispatch(getKidsFilterData(4));
    }
  };

  return (
    <div>
       <div style={{ width: "100%", height: "250px" }} className='upper_img_kids'>
        <h1>Kid's Apparels</h1>
      </div>
