import React from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { useNavigate } from 'react-router-dom'
import { removeUserCartItems } from '../Redux/Cart/action'
import "./Payment.css"

export const Payment = () => {

    const navigate = useNavigate()
    const dispatch = useDispatch()
    const {userId} = useSelector(state => state.login)
    const handlepayment = () => {
        alert("Payment Successful")
        dispatch(removeUserCartItems(userId))
        navigate("/")
    }


  return (
    <div className='payment_main_div'>
            <form onSubmit={handlepayment}>
        <div className='payment_details'>
            <h3>Payment</h3>
            <div className='direct_payment_img'>
                <div className='direct-pay'><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRsOpp2BPMTVcST_8SMh6BASoaO5fjSMZn8bQ&usqp=CAU" alt="" /></div>
                <div className='direct-pay'><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS9PdfjuRPQvPUv-fzMLCnxlfPIFdTANOA2s1YkYjVNgz5lFEUcJfhaQlLWH1SAtYmRaJs&usqp=CAU" alt="" /></div>
                <div className='direct-pay'><img src="https://www.mobigyaan.com/wp-content/uploads/2017/03/Paytm-logo.jpg" alt="" /></div>
                <div className='direct-pay'><img src="data:image/png;base64," alt="" /></div>
            </div>
                    <hr />
            <div className='card_details_billing_detail'>
                 <div className='billing_detail'>
                    <h3>Billing Details</h3>
                    
                    <input className='input' type="text" placeholder='Full Name'required/><br />
                   
                    <input className='input' type="text"  placeholder='Address'required/>
                    <div className='city_pin'>
                        
                        <input className='input'placeholder='City'  type="text" required/>
                        
                        <input className='input' placeholder='Pin' type="text" required/>
                    </div>
                    <input className='input' type="text" placeholder='Country'required/>
                 </div>
                 <div className='card_details'>
                 <h3>Card Details</h3>
                    
                    <input className='input' type="text" placeholder='Cardholder Name'required/><br />
                   
                    <input className='input' type="text"  placeholder='Card Number'required />
                    <div className='city_pin'>
                        
                        <select className='input' required>
                           <option value="---">Month</option>
                           <option value="1">01</option>
                           <option value="2">02</option>
                           <option value="3">03</option>
                           <option value="4">04</option>
                           <option value="5">05</option>
                           <option value="6">06</option>
                           <option value="7">07</option>
                           <option value="8">08</option>
                           <option value="9">09</option>
                           <option value="10">10</option>
                           <option value="11">11</option>
                           <option value="12">12</option>
                           
                        </select>
                        
                        <select className='input' required>
                            <option value="---">Expire Year</option>
                            <option value="2022">2022</option>
                            <option value="2023">2023</option>
                            <option value="2024">2024</option>
                            <option value="2025">2025</option>
                            <option value="2026">2026</option>
                            <option value="2027">2027</option>
                            <option value="2028">2028</option>
                        </select>
                    </div>
                    <input className='input' type="password" placeholder='CVV Number'required maxLength={3} />
                 </div>
            </div>
                 <div className='move_forword_div'>
                     <div className='goback_cart' onClick={()=>navigate("/checkout")}>{"<- Retrun to Checkout"}</div>
                     <div className='goback_cart'><button type='submit'>Make Payment</button></div>
                 </div>
        </div>
                 </form>
    </div>
  )
}
